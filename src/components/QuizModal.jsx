    ...
    <div className="modal-overlay">
      <div className="card modal-content" style={{ maxWidth: '500px' }}>
        ...
        <div className="quiz-body" style={{ padding: '20px 0' }}>
          <h3 style={{ marginBottom: '20px', lineHeight: '1.4' }}>{q.question}</h3>
          ...
          {showExplanation && (
            <div style={{ marginTop: '20px', padding: '16px', borderRadius: '8px', background: 'var(--accent-subtle)', border: '1px solid var(--primary-light)' }}>
              <p style={{ fontWeight: 'bold', marginBottom: '8px', color: selectedOption === q.correctAnswer ? '#10b981' : '#ef4444' }}>
                {selectedOption === q.correctAnswer ? '✨ Correct!' : '❌ Incorrect'}
              </p>
              <p style={{ fontSize: '0.9rem', lineHeight: '1.5' }}>{q.explanation}</p>
              <button 
